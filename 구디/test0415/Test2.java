...
class Player {
	...
	public String toString() {
		return rank + "등 : " +name + "("+card1+","+card2+"):" + getScore();
	}
}
public class Test2 {
...
//		}
	...
	public static void calcRank(List<Player>list) {
		...
		for (int i = 1; i < length; i++) {
			if (list.get(i).getScore() == list.get(i-1).getScore()) {
				if (i == 1) {
					prevRank = list.get(i -1).rank + 1;				
				}
				list.get(i-1).rank = prevRank;
				list.get(i).rank = prevRank;
			} else {
				prevRank = 1 +i;
				list.get(i).rank = prevRank;
			}
